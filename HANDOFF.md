...
- A11Y (all SAFE-ADDITIVE except modals) — [ ] icon-only buttons (×, ←, delete) need aria-label. [ ] clickable `<div className="card">` rows need role="button"+tabIndex+onKeyDown. [ ] click-to-cycle status pills (`<span>`) → `<button>`+aria-label. [ ] form `<label>`s not associated to inputs (no htmlFor/id) → wrap input in label. [ ] modals: role="dialog"/aria-modal + focus trap + Escape + restore focus (MODERATE).
