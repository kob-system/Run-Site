...
- NEXT: #5 visible sync-status — enhance the worker header pill ("✓ All saved" green / "Offline · 1 pending" amber, from existing isOnline + offlineEntry); low-logic, deploy build-verified. Then #6 "My Day" home = BANK locally for JP's verified AM review (new screen, can't live-verify the worker side; too risky to deploy unverified). POLICY: low-logic worker changes (copy/additive-reading-existing-state) → deploy build-verified; new-screen/new-data worker builds → bank for JP's verified session.
