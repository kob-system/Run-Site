...
- ✅ owner-health verified on bundle 407cf584 (the "Your jobs"/My-Day deploy is safe).
- ⚠️ #11 "+ Invoice this job" (Budget tab) — BUILT, but the FIRST deploy was a DEAD button: it set `showNewInvoice` from inside the job-detail `return`, but the invoice modal only renders in the MAIN `return` → modal never showed. **GOTCHA: modals live only in the main return, not the job-detail return; to open one from a job you must exit the job first.** FIXED in commit **a1651f4** (button now does `setSelectedProject(null)` + `setActiveTab('invoices')` so it lands on Invoices where the modal is, with the job pre-filled).
  - ⏳ BUT Vercel was SLOW (~6+ min) — a1651f4 NOT yet live; production still on bundle **e4045bdf** (= the dead-button version). **NEXT TICK, FIRST THING: re-check the live bundle — if it is no longer `e4045bdf`, open Kitchen Remodel → Money/Budget → click "+ Invoice this job" and confirm it lands on Invoices with the modal OPEN + the job pre-selected. If STILL `e4045bdf` after this, the deploy is stuck → push an empty commit to re-trigger, or check Vercel build logs.**
- Then: money-rounding (#7), voice input (#7), tests. POLICY: low-logic worker → deploy build-verified; new-write-path/new-screen → bank/do-with-JP.
- ⚠️ COMMIT-MSG GOTCHA: PowerShell here-string commit messages BREAK on inner DOUBLE-QUOTES (cost 2 retries tonight) — use NO double-quotes in commit messages.
