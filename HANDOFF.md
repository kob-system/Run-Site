...
- NEXT: #6 "My Day" worker home (research's #1-impact item = merge clock-in + today's job list + read-only job card [address / one-tap Get-Directions / notes / gate codes] + first-class Photo button). BANK locally (new worker screen, can't live-verify). Then voice input (#7), owner polish, tests. POLICY: low-logic worker changes → deploy build-verified; new-screen/new-data → bank for JP.
