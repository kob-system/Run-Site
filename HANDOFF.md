...
- NEXT: (a) quick owner-health check on the post-faea26d bundle; (b) **Supabase payroll-integrity (#9)** — a BEFORE INSERT/UPDATE trigger on `time_entries` that RECOMPUTES total_minutes + labor_cost from clock times + the worker's server-held `profiles.hourly_rate`, OVERWRITING whatever the client sent. NOTE: use a recompute trigger, NOT column locks — the trigger lets the worker's clock-out upsert still succeed (won't break the core flow) while making the values authoritative. Drive via Supabase SQL editor (Monaco base64 inject; the trigger is testable via a SQL insert→select). If injection is too finicky autonomously, BANK the .sql + tell JP to paste it (Supabase is open). Then #6 "My Day" = BANK for JP's verified review (new worker screen). POLICY: low-logic worker changes → deploy build-verified; new-screen/new-data → bank.
