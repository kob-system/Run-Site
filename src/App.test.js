...
test('a logged-out visitor at the root gets the public landing page', async () => {
  render(<App />)
  expect(await screen.findByRole('heading', { level: 1, name: /easy to find on Google/i }, ROUTE_LOAD)).toBeInTheDocument()
})
...
test('the installed icon never answers a crew member with the sales page', async () => {
  ...
  expect(screen.queryByRole('heading', { level: 1, name: /easy to find on Google/i })).not.toBeInTheDocument()
})
...
test('/jp, the referral link, renders with a tap-to-call phone and the new prices', async () => {
  goTo('/jp')
  render(<App />)
  expect(await screen.findByRole('heading', { name: /Tell us about your business/i }, ROUTE_LOAD)).toBeInTheDocument()
  ...
  // /jp shares the homepage headline on purpose; it is the short page, so the homepage's full pricing guarantee is not here.
  expect(screen.queryByText(/from the day we get your logins/i)).not.toBeInTheDocument()
})
...
test('storefront pages carry no em or en dashes and none of the banned owner-facing words', async () => {
  for (const path of ['/', '/jp']) {
    ...
    // "The Follow-Up System" and "Kobrossi Systems" are proper names; everything else is banned (CLAUDE.md §4).
    const scrubbed = text.replace(/Follow-Up System/g, '').replace(/Kobrossi Systems/g, '').replace(/Apple Maps/g, '')
    expect(scrubbed).not.toMatch(/\b(AI|software|apps?|platform|systems?|automation|POS)\b/i)
    unmount()
  }
})

test('/jp offers exactly three ways to reach us: call, text, email', async () => {
  goTo('/jp')
  render(<App />)
  await screen.findByRole('heading', { name: /Tell us about your business/i }, ROUTE_LOAD)
  const schemes = new Set(screen.getAllByRole('link').map((a) => (a.getAttribute('href') || '').split(':')[0]).filter((s) => ['tel', 'sms', 'mailto'].includes(s)))
  expect([...schemes].sort()).toEqual(['mailto', 'sms', 'tel'])
